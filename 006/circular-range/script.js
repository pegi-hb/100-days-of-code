...
document.addEventListener('mousemove', (e) => {
    if (isRotating) {
        ...
        progress.setAttribute('stroke-dasharray', `${percent * progressPerimeter} 10000`);

        thumb.setAttribute('cx', `${progressCx + Math.cos(angle) * progressRadius}`);
        thumb.setAttribute('cy', `${progressCx + Math.sin(angle) * progressRadius}`);
